...
LOG_PATH = OUTPUT_DIR / "parse_build_files.log"
...
def parse_one_file(file_path, old_style):
    """Runs the converter and returns the raw dict. Raises on failure."""
    with open(file_path, "r") as build_file:
        if old_style:
            build_dict = build_converter.convert_old_style_build_file(build_file)
        else:
...
def main():
    ...
    with open(RAW_RECORDS_PATH, "a", encoding="utf-8") as raw_out, \
        ...
        for file_path in sorted(BUILD_FILE_DIRECTORY.iterdir()):
